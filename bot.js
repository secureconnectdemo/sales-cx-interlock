...
app.post("/webhook", async (req, res) => {
    ...
    if (resource === "messages") {
      ...
      for (const line of lines) {
        if (line === "/submit deployment") {
          await axios.post("https://webexapis.com/v1/messages", { roomId, markdown: "📝 Opening the **Secure Access Deployment Form**...\n\n⌛ *Please wait a few seconds for the form to appear if the bot has been idle.*" }, { headers: { Authorization: WEBEX_BOT_TOKEN } });
          await sendForm(roomId, "deployment");
          return res.sendStatus(200);
        } else if (line === "/submit handoff") {
          await axios.post("https://webexapis.com/v1/messages", { roomId, markdown: "📋 Opening the **Secure Access Handoff Form**...\n\n⌛ *Please wait a few seconds for the form to appear if the bot has been idle.*" }, { headers: { Authorization: WEBEX_BOT_TOKEN } });
          await sendForm(roomId, "handoff");
          return res.sendStatus(200);
        } else if (line === "/help") {
          await axios.post("https://webexapis.com/v1/messages", { roomId, markdown: `🤖 **SSE-CX-Hub Bot – Help Menu**\n\`/submit deployment\` – Open Deployment Form  \n\`/submit handoff\` – Open Handoff Checklist  \n\`/reset\` – (Coming Soon)  \nContact: josfonse@cisco.com` }, { headers: { Authorization: WEBEX_BOT_TOKEN } });
          return res.sendStatus(200);
        } else if (line === "/reset") {
          await axios.post("https://webexapis.com/v1/messages", { roomId, markdown: "🔄 Reset acknowledged. (Coming soon.)" }, { headers: { Authorization: WEBEX_BOT_TOKEN } });
          return res.sendStatus(200);
        }
      }
  ...
  .filter(b => [
    ...
    "low-doc", "low-contact", "low-plan"
        ...
        await axios.post("https://webexapis.com/v1/messages", { roomId: data.roomId, markdown: confirmation }, { headers: { Authorization: WEBEX_BOT_TOKEN } });
      }
    }
